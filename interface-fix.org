1. Interface fixes:
   ...
   1) Need better ways of interacting once the interaction area is in
      motion. Example: not possible to draw a horizontal line.
   2) We should work on the overall UI design/layout. I am finding it
      very awkward attempting to use this on a tablet (eg, buttons are
      too small, too close together, etc)
   3) DONE: (gain to 0.5 while firing) You might want to set the Gain
      to the sound model a little lower since when so many notes are
      played at the same time, they distort.
      - Set gain to 0.25
      - Automatic rescaling
   4) NO: Do we still need a toggle for live interaction?
   5) DONE: We need to be able to scale the grid smaller/larger.
   6) DONE: Get the sound back in (user optional)
      - Turn sound on/off takes care of this.
   7) DONE: Have the CA compute and display out to the number of steps ahead
      that are visible.
      - The CA computes the number of steps till the edge of the screen
        at every time step. As the current state changes, the future
        cells are recomputed to reflect this change.
   8) DONE: Recompute out to the edge of visibility whenever a human
      switches a cells value.
      - The recomputation cycle happens at the beginning of every time
        step. So, technically, as per the implementation, the changes in
        recmoputation occurs only at the beginning of the next time
        step. However, it looks like the recomputation happens, whenever
        the human makes a change. On the other hand, I'm not sure why
        should the cells be recomputed everytime the human makes the
        change.
   9) DONE: Currently, reset does not do the right thing
      - Fixed reset to keep track of the initital configuration when
        start is run.
   10) DONE: It would be nice to have a "step" button
   11) DONE: "On" and "off" toggle buttons tend to be confusing unless it is
       clear whether they are indicating a) current state, or b) the
       action to take. My suggestion is to change the text as follows:
       "Turn sound [off/on]"
   12) DONE: How did we loose the nice clean attack and decay?
   13) DONE: Do the words on the button indicate the current state, or
       the state that the system would be in if the button were
       pushed. Can you fix this classical interface issue?
       - The sound toggle has been replaced by "turn sound" on/off.
   14) DONE: It would be great to control the rate (the time step) so
       one can see better what is happening if one wants.
       - The UI has a text box in the side that can be used to control
         the rate of the CA.
