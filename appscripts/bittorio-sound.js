...
document.getElementById('userGuide').innerHTML += "<ol> <li>The first (top) row is the initial configuration of the CA.</li> <li>Each subsequent row is the state of the CA in a subsequent time-step</li> <li>Future states can be grey, black, or white. Cells that are black or white are treated as 'perturbations' that are 'external' to the CA.</li> </ol>"
        ...
        var colLength = 36; // 3 octaves from 110 to 440
        ...
        //creates an oscillator assigned with a bittorio objecct
        function createBittorioOsc (x){
            var obj = {};
            obj.state = 0;
            obj.playing = 0;
            obj.sound = null;
            obj.freq = 110 * Math.pow(2, x/12);

            //initialize oscillator, needed as oscillators cant be turned off and then on
            obj.init = function(x){
                obj.sound = audioContext.createOscillator();
                obj.sound.freq = obj.freq; //110 is lowest frequency
                obj.sound.type = "sine";
            }
            ...
            obj.stop = function(){
                ...
                obj.playing = 0;
                obj.sound.stop(0); //stop immediately
                obj.sound.disconnect();
            }

            obj.play = function(){
                console.log("Starting" + x + "playing" + obj.playing);
                obj.sound = null;
                obj.init(); //play itself does everything
                obj.sound.connect(audioContext.destination);
                obj.sound.start(0);
                obj.playing = 1;
            }

            obj.updateState = function(caState){
                obj.state = caState;
                console.log("obj playing" + obj.playing);
                if(obj.state == 1 && obj.playing == 1){ //continue playing

                }
                else if(obj.state == 1 && obj.playing == 0){ //start playing
                    obj.play();
                }
                else if(obj.state == 0 && obj.playing == 1){ //stop playing
                    obj.stop();
                }
                else{
                    // do nothing
        ...
        function init(){
            ...
            for(col=0; col< colLength; col++){
                ...
                //create as many number of oscillator
                bitOsc[col]= new createBittorioOsc(col);
            }
        }
        ...
        function reset(){
            ...
            for(col=0; col< colLength; col++){
                bitOsc[col].sound = null;
                bitOsc[col].init(col);
                bitOsc[col].playing = 0;
                bitOsc[col].updateState(0);
            }
        }
        ...
        function caUpdate(){
            ...
            while( ind < arr.length ){
                ...
                bitOsc[ind].updateState(el.state);
